...
#define FLASH_UNSUPPORTED_ARCH                                                         \
    printf(                                                                            \
        "FATAL: FlashSparseAttention requires building with sm version sm80-sm90, "    \
        "but was built for < 8.0!"                                                     \
    );
...
DEFINE_FLASH_BACKWARD_KERNEL(
    ...
    FLASH_NAMESPACE::compute_dq_dk_dv<Kernel_traits, Is_causal, Is_even_M, Is_even_K>(
        params
    );
    #else
...
DEFINE_FLASH_BACKWARD_KERNEL(
    ...
    #if defined(ARCH_SUPPORTS_FLASH)
    FLASH_NAMESPACE::compute_dq_dk_dv_seqk_parallel<
        Kernel_traits,
        Is_causal,
        Is_even_MN,
        Is_even_K,
...
template <typename Kernel_traits>
__global__ void
flash_bwd_convert_dq_kernel(const Flash_bwd_params params, const int nsplits) {
...
void run_flash_bwd_seqk_parallel(Flash_bwd_params &params, cudaStream_t stream) {
    ...
    if (!params.deterministic) {
        flash_bwd_dot_do_o_kernel<true, Kernel_traits>
            <<<grid_m, Kernel_traits::kNThreads, 0, stream>>>(params);
    } else {
        flash_bwd_dot_do_o_kernel<false, Kernel_traits>
            <<<grid_m, Kernel_traits::kNThreads, 0, stream>>>(params);
    }
        ...
        EVENK_SWITCH(is_even_K, IsEvenKConst, [&] {
            SOFTCAP_SWITCH(params.softcap > 0.0, Is_softcap, [&] {
                ...
                auto kernel = &flash_bwd_dq_dk_dv_loop_seqk_parallel_kernel <
                    Kernel_traits,
                    Is_causal,
                    IsEvenMNConst && IsEvenKConst && Kernel_traits::kHeadDim <= 128,
                    IsEvenKConst, Is_softcap
                >; 
    ...
    kernel_dq<<<grid_m, Kernel_traits::kNThreads, Kernel_traits::kSmemdQSize, stream>>>(
        params,
        !params.deterministic ? 1 : gridDimx
    );
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
void run_mha_bwd_hdim32(Flash_bwd_params &params, cudaStream_t stream) {
    ...
    if (max_smem_per_block >= 104 * 1024) { // H100 and A100
        ...
        run_flash_bwd<
            Flash_bwd_kernel_traits<Headdim, 128, 128, 8, 4, 4, 4, false, false, T>,
            Is_causal
        >(params, stream);
    } else { // sm86 and sm89
        // 96KB, 1 CTAs in sm86 and sm 89.
        run_flash_bwd<
            Flash_bwd_kernel_traits<Headdim, 128, 128, 8, 4, 4, 4, false, true, T>,
            Is_causal
        >(params, stream);
    }
}
...
void run_mha_bwd_hdim64(Flash_bwd_params &params, cudaStream_t stream) {
    ...
    if (max_smem_per_block >= 144 * 1024) { // H100 and A100
        ...
        run_flash_bwd<
            Flash_bwd_kernel_traits<Headdim, 64, 128, 8, 2, 4, 4, false, false, T>,
            Is_causal
        >(params, stream);
    }
...
void run_mha_bwd_hdim96(Flash_bwd_params &params, cudaStream_t stream) {
    ...
    if (max_smem_per_block >= 116 * 1024) { // H100 and A100
        ...
        run_flash_bwd<
            Flash_bwd_kernel_traits<Headdim, 64, 64, 8, 2, 4, 4, false, false, T>,
            Is_causal
        >(params, stream);
    }
}
...
void run_mha_bwd_hdim128(Flash_bwd_params &params, cudaStream_t stream) {
    ...
    if (max_smem_per_block >= 144 * 1024) { // H100 and A100
        // 144KB, 1 CTAs in A100, 1 CTAs in H100.
        run_flash_bwd<
            Flash_bwd_kernel_traits<Headdim, 64, 128, 8, 2, 4, 2, false, false, T>,
            Is_causal
        >(params, stream);
    } else { // sm86 and sm89
        // 80KB, 1 CTAs in sm86 and sm 89.
        run_flash_bwd<
            Flash_bwd_kernel_traits<Headdim, 64, 64, 8, 4, 2, 2, false, true, T>,
            Is_causal
        >(params, stream);
    }
}
...
void run_mha_bwd_hdim192(Flash_bwd_params &params, cudaStream_t stream) {
    ...
    if (max_smem_per_block >= 136 * 1024) { // H100 and A100
        ...
        run_flash_bwd<
            Flash_bwd_kernel_traits<Headdim, 64, 64, 8, 4, 2, 2, false, false, T>,
            Is_causal
        >(params, stream);
    } else { // sm86 and sm89
        // 96KB, 1 CTAs in sm86 and sm 89.
        run_flash_bwd<
            Flash_bwd_kernel_traits<Headdim, 64, 64, 8, 4, 2, 2, true, true, T>,
            Is_causal
        >(params, stream);
    }
}
...
void run_mha_bwd_hdim256(Flash_bwd_params &params, cudaStream_t stream) {
    ...
    if (max_smem_per_block >= 176 * 1024) { // H100
        ...
        // 96KB, 1 CTAs in sm86 and sm 89.
        run_flash_bwd<
            Flash_bwd_kernel_traits<Headdim, 64, 32, 8, 4, 1, 2, true, true, T>,
            Is_causal
        >(params, stream);
    }
}
