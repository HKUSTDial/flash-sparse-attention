...
DEFINE_FLASH_FORWARD_KERNEL(
    ...
    FLASH_NAMESPACE::
        combine_attn_seqk_parallel<Kernel_traits, kBlockM, Log_max_splits, Is_even_K>(
            params
        );
}
            ...
            BOOL_SWITCH(return_softmax, ReturnSoftmaxConst, [&] {
                SOFTCAP_SWITCH(params.softcap > 0.0, Is_softcap, [&] {
                    ...
                    auto kernel = &flash_fwd_kernel <
                        ...
                        IsEvenMNConst && IsEvenKConst && !ReturnSoftmaxConst &&
                        Kernel_traits::kHeadDim <= 128,
                        IsEvenKConst && !ReturnSoftmaxConst, Is_softcap,
                        ReturnSoftmaxConst && !Is_softcap
                    >;
...
void run_flash_splitkv_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    if (params.num_splits > 1) {
        ...
        constexpr static int kBlockM = Kernel_traits::kHeadDim % 128 == 0
            ? 4
...
void run_mha_fwd_hdim128(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    // 80KB, 1CTAs in sm86 and sm 89, 2 CTAs in A100, 2 CTAs in H100.
    run_flash_fwd<
        Flash_fwd_kernel_traits<Headdim, 64, 64, 4, false, false, T>,
        Is_causal
    >(params, stream);
...
void run_mha_fwd_hdim256(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    if (max_smem_per_block >= 104 * 1024) {
        ...
        run_flash_fwd<
            Flash_fwd_kernel_traits<Headdim, 64, 64, 4, true, true, T>,
            Is_causal
        >(params, stream);
    }
}
