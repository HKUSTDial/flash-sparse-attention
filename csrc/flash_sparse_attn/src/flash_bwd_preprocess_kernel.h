...
inline __device__ void dot_do_o(
    Tensor<Engine0, Layout0> const &do_,
    Tensor<Engine0, Layout0> const &o,
    Tensor<Engine1, Layout1> &dP_sum,
    const int gdP_col_stride
) {
    ...
    Tensor do_reshaped = make_tensor(
        do_.data(),
        make_layout(
            get<1>(do_.layout()),
            make_layout(get<0>(do_.layout()), get<2>(do_.layout()))
        )
    );
    ...
    for (int mi = 0; mi < size<0>(do_reshaped); ++mi) {
        ...
        dP_sum_cur = FLASH_NAMESPACE::Allreduce<THREADS_PER_ROW>::run(dP_sum_cur, sum_op);
...
inline __device__ void compute_dot_do_o(const Params &params) {
    ...
    const index_t row_offset_dq_accum =
        ...
        (m_block * kBlockM + (params.cu_seqlens_q == nullptr ? 0 : 128ll * bidb)) *
            params.h * params.d_rounded +
    ...
    const index_t row_offset_dpsum =
        (params.unpadded_lse
             ? (bidh * (params.total_q + 128 * params.b) +
    ...
    Tensor cdO = make_identity_tensor(
        Shape<Int<kBlockM>, Int<kHeadDim>>{}
...
inline __device__ void clear_dKVaccum(const Params &params) {
    ...
    const index_t row_offset_dkv_accum =
        ((bidb * params.h_k + bidh) * params.seqlen_k_rounded + n_block * kBlockN) *
        params.d_rounded;

    Tensor gdKaccum = make_tensor(
        make_gmem_ptr(reinterpret_cast<ElementAccum *>(params.dk_accum_ptr) + row_offset_dkv_accum),
        Shape<Int<kBlockN>, Int<kHeadDim>>{},
        Stride<Int<kHeadDim>, _1>{}
    );
    Tensor gdVaccum = make_tensor(
        make_gmem_ptr(reinterpret_cast<ElementAccum *>(params.dv_accum_ptr) + row_offset_dkv_accum),
        Shape<Int<kBlockN>, Int<kHeadDim>>{},
        Stride<Int<kHeadDim>, _1>{}
    );
