...
inline __device__ void compute_attn_1rowblock(
    ...
    Tensor mB = make_tensor(
        make_gmem_ptr(
            reinterpret_cast<Element *>(params.b_ptr) +
            binfo.b_offset(params.b_batch_stride, params.b_row_stride, bidb)
        ),
        make_shape(params.h_b, binfo.actual_seqlen_q, binfo.actual_seqlen_k),
        make_stride(params.b_head_stride, params.b_row_stride, _1{})
    );
    Tensor gB = local_tile(
        mB(bidh / params.h_h_b_ratio, _, _),
    ...
    Tensor tBgB = gmem_thr_copy_BSP.partition_S(gB); // (BCPY, BCPY_M, BCPY_N, nblocksN)
    ...
    Tensor tSgS = thr_mma.partition_C(gP);                     // (MMA, MMA_M, MMA_N)
    ...
    auto smem_tiled_copy_B = make_tiled_copy_C(typename Kernel_traits::SmemCopyAtomBSP{}, tiled_mma);
    ...
    Tensor tBcB = gmem_thr_copy_BSP.partition_S(cB);    // (CCPY, CCPY_M, CCPY_N) -> (blk_m, blk_n)
    ...
    __syncthreads();
    // Do OR-reduce on the B to see if any active threads for current iteration.
    FLASH_NAMESPACE::or_reduce(tSsB, any_active, smem_thr_copy_B);
    ...
    // Init mask processor
    FLASH_NAMESPACE::Mask mask(binfo.actual_seqlen_k, binfo.actual_seqlen_q);
    ...
    for (int masking_step = 0; masking_step < n_masking_steps;
        ...
        if (any_active) {
            ...
            mask.template apply_mask<Is_causal>(
                acc_s,
                n_block * kBlockN,
        ...
        if (n_block > n_block_min) {
            ...
            FLASH_NAMESPACE::or_reduce(tSsB, any_active_next, smem_thr_copy_B);
        ...
        if (any_active) {
            ...
            first_processed_block
                ? softmax.template softmax</*Is_first=*/true, /*Check_inf=*/true>(
                      acc_s,
                      acc_o
                  )
                : softmax.template softmax</*Is_first=*/false, /*Check_inf=*/true>(
                      acc_s,
                      acc_o
                  );
...
inline __device__ void compute_attn_1rowblock_splitkv(
    ...
    if (n_block_min >= n_block_max) { // This also covers the case where n_block_max <= 0
        ...
        // Construct identity layout for sO
        Tensor cO = make_identity_tensor(
            make_shape(size<0>(gOaccum), size<1>(gOaccum))
        );  // (BLK_M,BLK_K) -> (blk_m,blk_k)
        // Repeat the partitioning with identity layouts
    ...
    const index_t row_offset_v = block_table == nullptr
        ...
        : block_table[block_table_idx] * params.v_batch_stride +
            ...
            (bidh / params.h_h_k_ratio) * params.v_head_stride;
    const index_t row_offset_b = (block_table == nullptr)
        ? binfo.b_offset(params.b_batch_stride, params.b_row_stride, bidb_cache) +
            (bidh / params.h_h_b_ratio) * params.b_head_stride +
            m_block * kBlockM * params.b_row_stride + (n_block_max - 1) * kBlockN
        : binfo.q_offset(/*batch_stride=*/index_t(0), params.b_row_stride, bidb_cache) +
            (bidh / params.h_h_b_ratio) * params.b_head_stride +
            m_block * kBlockM * params.b_row_stride +
            block_table[block_table_idx] * params.b_batch_stride + block_table_offset;
    ...
    Tensor tQcQ = gmem_thr_copy_QKV.partition_S(cQ);    // (ACPY, ACPY_M, ACPY_K) -> (blk_m, blk_k)
    Tensor tKVcKV = gmem_thr_copy_QKV.partition_S(cKV); // (BCPY, BCPY_N, BCPY_K) -> (blk_n, blk_k)
    Tensor tBcB = gmem_thr_copy_BSP.partition_S(cB);    // (BCPY, BCPY_M, BCPY_N) -> (blk_m, blk_n)

    // Allocate predicate tensors for k
    Tensor tQpQ = make_tensor<bool>(make_shape(size<2>(tQsQ)));
    Tensor tKVpKV = make_tensor<bool>(make_shape(size<2>(tKsK)));
    // Allocate predicate tensors for n bounds
    Tensor tBpB = make_tensor<bool>(make_shape(size<2>(tBcB)));
    ...
    if (!Is_even_MN) {
        #pragma unroll
        for (int n = 0; n < size(tBpB); ++n) {
            tBpB(n) = get<1>(tBcB(0, 0, n)) < binfo.actual_seqlen_k - n_block * kBlockN;
        }
    }
    ...
    bool any_active = true;      // to be updated later for current iteration
    bool any_active_next = true; // Load the first Q, K, V, B tiles
    ...
    for (int masking_step = 0; masking_step < n_masking_steps;
        ...
        if (masking_step > 0) {
            if (block_table == nullptr) {
                ...
                const int block_table_idx_cur =
                    (n_block + 1) * kBlockN / params.page_block_size;
                const int block_table_offset_cur = (n_block + 1) * kBlockN -
                    block_table_idx_cur * params.page_block_size;
                const int block_table_idx_next =
                    n_block * kBlockN / params.page_block_size;
                const int block_table_offset_next =
                    n_block * kBlockN - block_table_idx_next * params.page_block_size;
                tVgV.data() = tVgV.data() +
                    (block_table[block_table_idx_next] -
                     block_table[block_table_idx_cur]) *
                        params.v_batch_stride +
                    (block_table_offset_next - block_table_offset_cur) *
                        params.v_row_stride;
            }
            if (any_active) {
                FLASH_NAMESPACE::copy</*Is_even_MN=*/true, Is_even_K>(
        ...
        if (n_block > n_block_min) {
            ...
            if (block_table == nullptr) {
                ...
                tBgB.data() = tBgB.data() + (-int(kBlockN));
            } else {
                const int block_table_idx_cur =
                    n_block * kBlockN / params.page_block_size;
                ...
                const int block_table_idx_next =
                    (n_block - 1) * kBlockN / params.page_block_size;
                const int block_table_offset_next = (n_block - 1) * kBlockN -
                    block_table_idx_next * params.page_block_size;
                tKgK.data() = tKgK.data() +
                    (block_table[block_table_idx_next] -
                     block_table[block_table_idx_cur]) *
    ...
    for (; n_block >= n_block_min; --n_block) {
        ...
        if (block_table == nullptr) {
            ...
            const int block_table_offset_next =
                n_block * kBlockN - block_table_idx_next * params.page_block_size;
            tVgV.data() = tVgV.data() +
                (block_table[block_table_idx_next] - block_table[block_table_idx_cur]) *
                    params.v_batch_stride +
                (block_table_offset_next - block_table_offset_cur) *
                    params.v_row_stride;
        }
    ...
    Tensor sOaccum = make_tensor(
        make_smem_ptr(reinterpret_cast<ElementO *>(smem_)),
        typename Kernel_traits::SmemLayoutO{}
    );  // (SMEM_M, SMEM_N)
    // Partition sO to match the accumulator partitioning
...
inline __device__ void combine_attn_seqk_parallel(const Params &params) {
    ...
    static_assert(
        kBlockM == 4 || kBlockM == 8 || kBlockM == 16 || kBlockM == 32,
    ...
    Tensor gLSE = make_tensor(
        make_gmem_ptr(
            reinterpret_cast<ElementAccum *>(params.softmax_lse_ptr) + row_offset_lse
        ),
        Shape<Int<kBlockM>>{},
        Stride<_1>{}
    );
    ...
    auto transposed_stride = params.seqlenq_ngroups_swapped
