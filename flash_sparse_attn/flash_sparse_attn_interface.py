...
def _flash_sparse_attn_forward(
    ...
    q, k, v, b = [maybe_contiguous(x) for x in (q, k, v, b)]
...
def _flash_sparse_attn_backward_fake(
    ...
    dout, q, k, v, b, out = [maybe_contiguous(x) for x in (dout, q, k, v, b, out)]
    if dq is None:
...
class FlashSparseAttnFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        if b is not None and b.shape[-1] != seqlen_k_rounded:
            if b.shape[-1] == 1:
                b = b.expand(*b.shape[:-1], seqlen_k_rounded)
            else:
                b = torch.nn.functional.pad(b, [0, seqlen_k_rounded - b.shape[-1]])
        ...
        if is_grad:
            ctx.save_for_backward(q, k, v, b, out_padded, softmax_lse)
            ctx.softmax_scale = softmax_scale
            ctx.is_causal = is_causal
            ctx.softcap = softcap
            ctx.deterministic = deterministic
            ctx.seqlen_k_b = seqlen_k_b
    ...
    def backward(
        ...
        *args: Any,
        ...
        q, k, v, b, out, softmax_lse = ctx.saved_tensors
        dq, dk, dv, db = (
            torch.zeros_like(q),
            torch.zeros_like(k),
            torch.zeros_like(v),
        ...
        if db is not None:
            db = (
                db[..., : k.shape[1]].sum(dim=-1, keepdim=True)
                if ctx.seqlen_k_b == 1
                else db[..., : k.shape[1]]
            )
...
def flash_sparse_attn_func(
    ...
    Similarity, also supports attn_bias with head dimension of 1, nheads_k or nheads for MQA/GQA.
    For example, if Q has 6 heads, K, V have 2 heads, then attn_bias can have head dimension
    of 1, 2 or 6. If it is 1, all heads use the same bias; if it is 2, head 0, 1, 2 of Q use head 0
    of bias, head 3, 4, 5 of Q use head 1 of bias. If it is 6, each head uses its own bias.
    ...
    Arguments:
        ...
        attn_bias: torch.Tensor. The attention bias float tensor of
