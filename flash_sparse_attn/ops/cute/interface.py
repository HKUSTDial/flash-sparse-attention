...
from flash_sparse_attn.ops.cute.cache_utils import get_jit_cache
from flash_sparse_attn.ops.cute.testing import is_fake_mode


if os.environ.get("CUTE_DSL_PTXAS_PATH", None) is not None:
    from flash_sparse_attn.ops.cute import cute_dsl_ptxas  # noqa: F401
...
from flash_sparse_attn.ops.cute import fa_logging
from flash_sparse_attn.ops.cute.cute_dsl_utils import (
...
from flash_sparse_attn.ops.cute.flash_bwd_postprocess import (
...
from flash_sparse_attn.ops.cute.prepare_scheduler import (
    FlashPrepareScheduler,
    SchedulerMetadataTensorsTorch,
)
from flash_sparse_attn.ops.cute.cu_blocks_kernel import (
...
BIN_BATCH_SEARCH_THRESH = (
    256  # above this batch size SingleTileVarlenScheduler gets a batch-lookup aid
)
...
@lru_cache(maxsize=None)
def _validate_head_dims(head_dim: int, compute_capability: int, alignment: int) -> None:
    """Validate head dimension constraints based on compute capability."""
    is_standard_range = 8 <= head_dim <= 128
    is_sm90_range = 8 <= head_dim <= 256
    if compute_capability == 9:
        assert is_sm90_range and head_dim % alignment == 0, (
            f"head_dim={head_dim} is not supported on SM90. "
            f"head_dim must be between 8 and 256 and divisible by {alignment}."
        )
    elif compute_capability in [10, 11]:
        assert is_standard_range and head_dim % alignment == 0, (
            f"head_dim={head_dim} is not supported on SM100/SM110. "
            f"head_dim must be between 8 and 128 and divisible by {alignment}."
        )
...
def _tile_size_fwd_sm90(head_dim, is_causal, is_local, sparse_block_size_q=None):
    ...
    if head_dim <= 64:
        ...
        tile_n = 96 if is_local else 112
...
def _tile_size_bwd_sm90(head_dim, causal, local, sparse_block_size_q=None):
    ...
    *,
    ...
    if tile_mn is None:
        if arch // 10 == 12:
            ...
            cfg = _tile_size_fwd_sm90(head_dim, causal, local, sparse_block_size_q=sparse_q)
    else:
...
def _flash_attn_fwd(
    ...
    requires_grad = any(t is not None and t.requires_grad for t in (q, k, v, learnable_sink))
    ...
    if cu_seqlens_k is None:
        if page_table is None:
            ...
            assert v.shape == (batch_size, seqlen_k, num_head_kv, head_dim)
        else:
            assert k.shape == (num_pages, page_size, num_head_kv, head_dim)
            assert v.shape == (num_pages, page_size, num_head_kv, head_dim)
    else:
        assert k.shape == (seqlen_k, num_head_kv, head_dim)
        assert v.shape == (seqlen_k, num_head_kv, head_dim)
    ...
    assert all(t is None or t.dtype == v.dtype for t in (q, k)), "q, k, v must have the same dtype"
    ...
    if arch // 10 in [10, 11] and head_dim == 256:
        raise NotImplementedError("SM100/SM110 forward with head_dim=256 is not supported")
    assert num_head % num_head_kv == 0, "num_head must be divisible by num_head_kv"
    alignment = 16 // v.element_size()
    if arch // 10 not in [8, 12]:
        _validate_head_dims(head_dim, arch // 10, alignment)
    ...
    if out is None:
        out = torch.empty(
            *q_batch_seqlen_shape, num_head, head_dim, dtype=out_torch_dtype, device=device
        )
    else:
        _validate_tensor(
            out,
            "out",
            (*q_batch_seqlen_shape, num_head, head_dim),
            out_torch_dtype,
            device,
        )
    ...
    if use_block_sparsity:
        ...
        if cu_seqlens_q is not None:
            ...
            if block_sparse_tensors.cu_block_idx_offsets is None and (
    ...
    # Scheduler metadata for SM100/SM110
    reuse_scheduler_metadata = scheduler_metadata is not None
    is_varlen_q = cu_seqlens_q is not None or seqused_q is not None
    cluster_shape_m = 2 if use_2cta_instrs else 1
    use_dedicated_hd256_kernel = False  # Not used in current code, placeholder for future
    ...
    # Binary batch search hint for varlen
    ...
    if use_cu_hint and has_scheduler_metadata and scheduler_metadata.cu_total_m_blocks is not None:
    ...
    is_static_persistent = (
        not causal and not local and cu_seqlens_q is None and seqused_q is None and not is_split_kv
    ) or (not torch.is_tensor(max_m_blocks_leq_one) and max_m_blocks_leq_one and not is_split_kv)
    ...
    compile_key = (
        ...
        (torch2cute_dtype_map[learnable_sink.dtype] if learnable_sink is not None else None),
    ...
    if not fake_mode:
        ...
        if is_fp8:
            ...
            q_call, k_call, v_call = [t.view(torch.uint8) for t in (q_call, k_call, v_call)]
        descale_tensors = (
...
def make_fake_bwd_tensors(dtype, has_gqa, varlen_q, varlen_k):
    ...
    b, seqlen_q, seqlen_k, h_q, d = sym(), sym(), sym(), sym(), sym()
    ...
    mO = fake_tensor(dtype, (*b_seqlenq, h_q, d), divisibility=div)
    mdO = fake_tensor(dtype, (*b_seqlenq, h_q, d), divisibility=div)
    mK = fake_tensor(dtype, (*b_seqlenk, h_kv, d), divisibility=div)
    mV = fake_tensor(dtype, (*b_seqlenk, h_kv, d), divisibility=div)
    mdQ = fake_tensor(dtype, (*b_seqlenq, h_q, d), divisibility=div)
    mdK = fake_tensor(dtype, (*b_seqlenk, h_kv, d), divisibility=div)
    mdV = fake_tensor(dtype, (*b_seqlenk, h_kv, d), divisibility=div)
...
def _compile_bwd_preprocess(
    ...
    mCuTotalMBlocks = (
        fake_tensor(Int32, (batchp1,), divisibility=1) if has_cu_total_m_blocks else None
    )
...
def _compile_bwd_postprocess(
    ...
    (
        ...
        mdKaccum,
        mdVaccum,
    ) = make_fake_bwd_tensors(dtype, has_gqa=True, varlen_q=has_cuseqlens_q, varlen_k=False)
    batch = mQ.shape[0] if not has_cuseqlens_q else cute.sym_int()
    batchp1 = cute.sym_int()
    mCuSeqlensQ = fake_tensor(Int32, (batchp1,), divisibility=1) if has_cuseqlens_q else None
    mSeqUsedQ = fake_tensor(Int32, (batch,), divisibility=1) if has_seqused_q else None
...
def _flash_attn_bwd(
    ...
    if block_sparse_tensors is not None:
        ...
        if learnable_sink is not None:
            assert arch // 10 in [9, 10, 11], (
                "Learnable sink backward is supported on SM90 and SM100/SM110"
            )
    ...
    if arch // 10 == 8:
        ...
        use_2cta_instrs = False
        num_threads = 128
        assert not (block_sparse_tensors is not None), (
    ...
    single_q_block = (not torch.is_tensor(seqlen_q_rounded)) and (
        seqlen_q_rounded // m_block_size == 1
    )
    single_k_block = (not torch.is_tensor(seqlen_k_rounded)) and (
    ...
    if cu_seqlens_k is None:
        ...
        assert v.shape == (total_k, num_head_kv, head_dim)
        assert cu_seqlens_k.shape == (batch_size + 1,), (
            "cu_seqlens_k must have shape (batch_size + 1,)"
        )
    ...
    if cu_seqlens_q is not None:
        ...
        assert out.shape == (total_q, num_head, head_dim)
        assert dout.shape == (total_q, num_head, head_dim)
        assert lse.shape == (num_head, total_q), "lse must have shape (num_head, total_q)"
    else:
        assert out.shape == (batch_size, seqlen_q, num_head, head_dim)
        assert dout.shape == (batch_size, seqlen_q, num_head, head_dim)
    ...
    if compile_key not in _flash_attn_bwd.compile_cache:
        current_stream = cute.runtime.make_fake_stream(use_tvm_ffi_env_stream=True)
        cute_aux_tensors = (
            [to_cute_aux_tensor(buf) for buf in aux_tensors] if aux_tensors is not None else None
        )
...
def _compile_fwd_combine(
    ...
    has_seqused,
    has_lse,
    has_virtual_batch_idx,
    has_num_splits_dynamic,
    has_semaphore_to_reset,
):
    ...
    mNumSplitsDynamic = (
        fake_tensor(Int32, (batch_for_1d,), divisibility=1) if has_num_splits_dynamic else None
    )
    mVirtualBatchIdx = (
        fake_tensor(Int32, (batch_for_1d,), divisibility=1) if has_virtual_batch_idx else None
    )
...
def _resolve_causal_local_window(causal, window_size_left, window_size_right):
    ...
    if causal:
        ...
        window_size_right = 0
        local = False
    elif window_size_left is not None or window_size_right is not None:
        causal = False
        local = True
...
def _get_scheduler_metadata(
    ...
    has_varlen_info = cu_seqlens_q is not None or seqused_q is not None
    needs_compute_tile_cumsum = (
        has_varlen_info and num_batch > BIN_BATCH_SEARCH_THRESH and tile_count_semaphore is None
    )
