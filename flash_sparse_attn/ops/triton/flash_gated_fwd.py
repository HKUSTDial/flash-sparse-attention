...
def _fwd_gated_kernel(
    ...
    if not IS_LOCAL and n_block_max_no_mask > block_sched.n_block_min:
        ...
        # Load delta tile
        d_tile = ptrs_sched.load_d(config, d_ptrs, n_block_max_no_mask - 1)
        d_max = tl.max(d_tile)
        d_min = tl.min(d_tile)
...
def _flash_gated_attn_forward(
    ...
    kernel_name = f"fwd_gated{'_split' if is_split_kv else ''}"
    launch_config = None
    if not is_autotune:
    ...
    kernel[grid](
        ...
        query.stride(2),
        query.stride(0),
        key.stride(0) if page_table is not None else key.stride(1),
        key.stride(2),
        key.stride(1) if page_table is not None else key.stride(0),
        value.stride(0) if page_table is not None else value.stride(1),
        value.stride(2),
        value.stride(1) if page_table is not None else value.stride(0),
        alpha.stride(0),
        alpha.stride(1),
        alpha.stride(-1),
        delta.stride(0),
