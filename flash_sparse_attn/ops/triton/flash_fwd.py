...
def _fwd_inner_sm90_kernel(
    ...
    # Load value tile
    # v_tile = tl.load_tensor_descriptor(v_ptrs, offsets=[n_block * TILE_N, 0])
    v_tile = tl.load(v_ptrs, boundary_check=(0, 1))
    ...
    if n_block > n_block_min:
        # Load next key tile
        # k_tile = tl.load_tensor_descriptor(k_ptrs, offsets=[(n_block - 1) * TILE_N, 0])
        k_tile = tl.load(k_ptrs, boundary_check=(0, 1))
...
def _fwd_sm90_kernel(
    ...
    # k_ptrs = tl.make_tensor_descriptor(
    #     base=k_base,
    #     shape=[actual_seqlen_k, head_dim],
    #     strides=[stride_kn, 1],
    #     block_shape=[TILE_N, TILE_K],
    ...
    # v_ptrs = tl.make_tensor_descriptor(
    #     base=v_base,
    #     shape=[actual_seqlen_k, head_dim],
    #     strides=[stride_vn, 1],
    #     block_shape=[TILE_N, TILE_K],
    ...
    if PACK_GQA:
        ...
        # q_tile = tl.load_tensor_descriptor(q_ptrs, offsets=[m_block * TILE_M, 0])
        q_tile = tl.load(q_ptrs, boundary_check=(0, 1))
    ...
    # Load key tile
    # k_tile = tl.load_tensor_descriptor(k_ptrs, offsets=[(n_block_max - 1) * TILE_N, 0])
    k_tile = tl.load(k_ptrs, boundary_check=(0, 1))
    ...
    if IS_CAUSAL or IS_LOCAL:
        ...
        # TODO: In Triton 3.6, the warp specialization heuristics are not optimal for this loop, so we disable it for now.
        # We should re-enable it once the heuristics are improved.
        for n_block in tl.range(
            n_block_max - 1, n_block_min_causal_local - 1, -1, warp_specialize=False
        ):
            k_tile, k_ptrs, v_ptrs, acc_o, row_max, row_sum = _fwd_inner_sm90_kernel(
                q_tile=q_tile,
                k_tile=k_tile,
                k_ptrs=k_ptrs,
                v_ptrs=v_ptrs,
    ...
    # Process n_blocks without masking
    for n_block in tl.range(
        n_block_max_no_mask - 1, n_block_min - 1, -1, warp_specialize=False
    ):
        k_tile, k_ptrs, v_ptrs, acc_o, row_max, row_sum = _fwd_inner_sm90_kernel(
            q_tile=q_tile,
            k_tile=k_tile,
...
def _flash_attn_varlen_sm90_forward(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    cu_seqlens_q: torch.Tensor,
