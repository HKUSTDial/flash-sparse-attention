...
def _run_base_topk_case(
    ...
    delta = torch.randn(seqlen_k, batch_size, num_heads_kv, device=device, dtype=dtype)
    ...
    k_topk = _gather_topk_3d(k.transpose(0, 1), gather_kv_indices)
    v_topk = _gather_topk_3d(v.transpose(0, 1), gather_kv_indices)
    delta_topk = _gather_topk_2d(delta.transpose(0, 1), gather_kv_indices)
